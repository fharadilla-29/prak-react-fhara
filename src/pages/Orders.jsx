  ...
  const fetchOrders = async () => {
    try {
      const { data, error } = await supabase
        .from('orders')
        .select('*, users(full_name), order_items(*, products(name))')
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-lg">
            ...
            <div className="space-y-3">
              ...
              <p>Processing: <span className="font-bold">{orders.filter(o => o.status === 'Processing').length}</span></p>
              <p>Completed: <span className="font-bold">{orders.filter(o => o.status === 'Completed').length}</span></p>
